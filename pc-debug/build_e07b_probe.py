"""Build only the E07b diagnostic; no cIOS build, installation or SD writes."""
from pathlib import Path
import subprocess, hashlib, struct
root=Path(__file__).resolve().parents[1]
out=root/'work/e07b-build'
...
run([cc,*arch,'-O2','-Wall','-Wextra','-Werror','-I'+str(sdk/'libogc/include'),
     '-c',root/'probe/diagnostics/e07b.c','-o',out/'probe.o'])
run([cc,*arch,out/'probe.o','-L'+str(sdk/'libogc/lib/wii'),'-logc','-lm','-o',out/'e07b-probe.elf'])
run([sdk/'tools/bin/elf2dol.exe',out/'e07b-probe.elf',out/'e07b-boot.dol'])
data=(out/'e07b-boot.dol').read_bytes()
assert b'E07b GIANTS VERSION BUFFER - test 2' in data
