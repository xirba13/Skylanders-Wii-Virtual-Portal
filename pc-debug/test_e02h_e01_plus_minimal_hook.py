#!/usr/bin/env python3
"""Local gate for E02h: proven E01 GETVERSION plus proven E02g hook."""
...
def main() -> int:
    ...
    hook = function(disassembly, "hid_receive_guarded_passthrough")
    checks = []
    ...
    checks.append(check("dos callsites E02g", all(bytes.fromhex(x) in blob for x in ("13658288", "136582f4")), "hook público"))
    checks.append(check("hook de 8 bytes", "push\t{r4, lr}" in hook and "pop" in hook, "mismo marco mínimo"))
    checks.append(check("una recepción", source.count("os_message_queue_receive(") == 1, "sin bucle"))
    forbidden = ("GETDEVICECHANGE", "GETDEVPARAMS", "HID_QUEUE_ID", "pending_device_change", "portal_announced")
    checks.append(check("sin fases posteriores", all(x not in source for x in forbidden), "sin enumeración ni estado"))
    ...
    print(f"[INFO] SHA-256 E02h: {digest}")
    print(f"\nE02h local gate: {sum(checks)}/{len(checks)} pruebas correctas")
