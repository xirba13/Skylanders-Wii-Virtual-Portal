"""Build only the E07 diagnostic; no cIOS build, installation or SD writes."""
from pathlib import Path
import subprocess, hashlib, struct
root=Path(__file__).resolve().parents[1]
out=root/'work/e07-build'
...
run([cc,*arch,'-O2','-Wall','-Wextra','-Werror','-I'+str(sdk/'libogc/include'),
     '-c',root/'probe/diagnostics/e07.c','-o',out/'probe.o'])
run([cc,*arch,out/'probe.o','-L'+str(sdk/'libogc/lib/wii'),'-logc','-lm','-o',out/'e07-probe.elf'])
run([sdk/'tools/bin/elf2dol.exe',out/'e07-probe.elf',out/'e07-boot.dol'])
data=(out/'e07-boot.dol').read_bytes()
assert b'E07 HIDv4 EMPTY PORTAL - test 1' in data
