"""Build only the E06c diagnostic; no cIOS build, installation or SD writes."""
from pathlib import Path
import subprocess, hashlib, struct
root=Path(__file__).resolve().parents[1]
out=root/'work/e06c-build'
...
run([cc,*arch,'-O2','-Wall','-Wextra','-Werror','-I'+str(sdk/'libogc/include'),
     '-c',root/'probe/diagnostics/e06c.c','-o',out/'probe.o'])
run([cc,*arch,out/'probe.o','-L'+str(sdk/'libogc/lib/wii'),'-logc','-lm','-o',out/'probe.elf'])
run([sdk/'tools/bin/elf2dol.exe',out/'probe.elf',out/'boot.dol'])
data=(out/'boot.dol').read_bytes()
assert b'E06c-bootstrap-diagnostic-v33' in data
