"""Build only the E08c diagnostic; no cIOS build, installation or SD writes."""
from pathlib import Path
import subprocess, hashlib, struct
root=Path(__file__).resolve().parents[1]
out=root/'work/e08c-build'
...
run([cc,*arch,'-O2','-Wall','-Wextra','-Werror','-I'+str(sdk/'libogc/include'),
     '-c',root/'probe/diagnostics/e08c.c','-o',out/'probe.o'])
run([cc,*arch,out/'probe.o','-L'+str(sdk/'libogc/lib/wii'),'-logc','-lm','-o',out/'e08c-probe.elf'])
run([sdk/'tools/bin/elf2dol.exe',out/'e08c-probe.elf',out/'e08c-boot.dol'])
data=(out/'e08c-boot.dol').read_bytes()
assert b'E08c CHARACTER TIMING' in data
